...
#autoselectQM_cube select Ti as a cube shape
...
proc autoselectQM { centerid rmin t_ele buffer T attempt} {
  ...
  set QMo [ atomselect top "type $t_ele and (not within $rmin of index $centerid ) and within $buffer of (index $id_QMi)" ]
  set n_QMo [ $QMo num ]
...
proc autoselectQM3 { centerid QM1_def t_ele buffer coord T attempt } {
  ...
  #set outer sphere
  set QMo [ atomselect top "type $t_ele and (not index $id_QMi ) and (within $buffer of (not type $t_ele and index $id_QMi))" ]
  set n_QMo [ $QMo num ]
  set id_QMo [ $QMo get index ]
  puts "known atoms $id_QMi "
  puts "select atoms from $id_QMo"
...
proc autoselectQM_cube { centerid rmin t_ele buffer T attempt } {
  ...
  set fo_config [ open [format "$stat-$T.config" ] "w"]
  #repeats the argument
  puts "original command: autoselectQM_cube $centerid $rmin $t_ele $buffer $T $attempt "
  puts "$stat netcharge $Q_QMi"
  puts $fo_info "original command: autoselectQM_cube $centerid $rmin $t_ele $buffer $T $attempt "
  puts $fo_info "$stat netcharge $Q_QMi"
