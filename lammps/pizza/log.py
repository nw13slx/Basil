...
class log:
    ...
    def __init__(self, *list, verbose=False):
        ...
        if len(list) == 1:
            ...
            if len(self.flist) > 1:
                raise RuntimeError("can only incrementally read one log file")
            self.increment = 1
            self.eof = 0
    ...
    def read_all(self):
        ...
        if self.nvec == 0:
            raise RuntimeError("log file has no values")
        ...
        print("read %d log entries" % self.nlen, file=self.fout)
    ...
    def get(self, *keys):
        if len(keys) == 0:
            raise RuntimeError("no log vectors specified")
