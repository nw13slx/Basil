...
class dump:
    ...
    def read_all(self):
        ...
        for file in self.flist:
            ...
            while snap:
                ...
                print(snap.time, end=' ', flush=True, file=self.fout)
                snap = self.read_snapshot(f)
        ...
        if len(self.names):
            print("assigned columns:", self.names2str(), file=self.fout)
        else:
            print("no column assignments made", file=self.fout)
        ...
        if ("x" not in self.names) or \
           ...
           ("z" not in self.names):
            print("dump scaling status is unknown", file=self.fout)
        elif self.nsnaps > 0:
            if self.scale_original == 1:
                self.unscale()
            elif self.scale_original == 0:
                print("dump is already unscaled", file=self.fout)
            else:
                print("dump scaling status is unknown", file=self.fout)
            ...
            if len(self.names) == 0:
                ...
                if len(words):
                    for i, word in enumerate(words):
                        if word == "x" or word == "xu":
                            xflag = 0
                            self.names["x"] = i
                        elif word == "xs" or word == "xsu":
                            xflag = 1
                            self.names["x"] = i
                        elif word == "y" or word == "yu":
                            yflag = 0
                            self.names["y"] = i
                        elif word == "ys" or word == "ysu":
                            yflag = 1
                            self.names["y"] = i
                        elif word == "z" or word == "zu":
                            zflag = 0
                            self.names["z"] = i
                        elif word == "zs" or word == "zsu":
                            zflag = 1
                            self.names["z"] = i
                        else:
                            self.names[word] = i
    ...
    def map(self, *pairs):
        if len(pairs) % 2 != 0:
            raise RuntimeError("dump map() requires pairs of mappings")
    ...
    def delete(self):
        ...
        while i < self.nsnaps:
            if not self.snaps[i].tselect:
                ...
                i += 1
        print("%d snapshots deleted" % ndel, file=self.fout)
        print("%d snapshots remaining" % self.nsnaps, file=self.fout)
    ...
    def scale(self, *list):
        if len(list) == 0:
            print("Scaling dump ...", file=self.fout)
            x = self.names["x"]
            y = self.names["y"]
            z = self.names["z"]
    ...
    def unscale(self, *list):
        if len(list) == 0:
            print("Unscaling dump ...", file=self.fout)
            x = self.names["x"]
            y = self.names["y"]
            z = self.names["z"]
            for snap in self.snaps:
    ...
    def wrap(self):
        print("Wrapping dump ...", file=self.fout)
    ...
    def unwrap(self):
        print("Unwrapping dump ...", file=self.fout)
    ...
    def owrap(self, other):
        print("Wrapping to other ...", file=self.fout)
        ...
        for snap in self.snaps:
            ...
            if self.lineflag == 2 or self.triflag == 2:
                self.objextra.owrap(snap.time, xprd, yprd, zprd,
                                    ids, atoms, iother, ix, iy, iz)
    ...
    def sort(self, *list):
        if len(list) == 0:
            print("Sorting selected snapshots ...", file=self.fout)
            id = self.names["id"]
            for snap in self.snaps:
                if snap.tselect:
                    self.sort_one(snap, id)
        elif type(list[0]) is types.StringType:
            print("Sorting selected snapshots by %s ..." % list[0], file=self.fout)
    ...
    def write(self, file, header=1, append=0):
        ...
        for snap in self.snaps:
            ...
            for i in range(snap.natoms):
                ...
                print(line, file=f)
        f.close()
        print("\n%d snapshots" % self.nselect, file=self.fout)
    ...
    def spread(self, old, n, new):
        ...
        print("min/max = ", min, max, file=self.fout)
    ...
    def atom(self, n, *list):
        if len(list) == 0:
            raise RuntimeError("no columns specified")
        columns = []
        values = []
        for name in list:
        ...
        for snap in self.snaps:
            ...
            for i in range(snap.natoms):
                if atoms[i][id] == n:
                    break
            if atoms[i][id] != n:
                raise RuntimeError("could not find atom ID in snapshot")
            for j in range(ncol):
    ...
    def findtime(self, n):
        for i in range(self.nsnaps):
            if self.snaps[i].time == n:
                return i
        raise RuntimeError("no step %d exists" % n)
    ...
    def extra(self, arg):
        ...
        if type(arg) is types.InstanceType and ".data" in str(arg.__class__):
            ...
            try:
                ...
                raise RuntimeError("could not extract bonds from data object")
        ...
        # tdump object, grab tris dynamically
            ...
            raise RuntimeError("unrecognized argument to dump.extra()")
...
class tselect:
    ...
    def all(self):
        ...
        data.aselect.all()
        print("%d snapshots selected out of %d" % (data.nselect, data.nsnaps), file=self.fout)
            ...
            for i in range(data.nsnaps):
                if data.snaps[i].tselect:
                    print("%d atoms of %d selected in first step %d" %
                          (data.snaps[i].nselect, data.snaps[i].natoms, data.snaps[i].time), file=self.fout)
                    break
